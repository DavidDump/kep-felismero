...
class ProcessedImage<TColor, TDepth> {
    // the current working image the operations will work on
    Image<TColor, TDepth> currentImage;
    
    // the mask generated by inrage operation
    // Image<Gray, TDepth> rangeMask;
    ...
    public ProcessedImage<TColor, TDepth>(string filepath) {
        this.currentImage = new Image<TColor, TDepth>(filepath);
    }

    public ProcessedImage<TColor, TDepth> SmoothGaussian(int kernelSize) {
        this.currentImage.SmoothGaussian(kernelSize);
        return this;
    }

    public ProcessedImage<TColor, TDepth> InRange(TColor bot, TColor top) {
        this.currentImage = this.currentImage.InRange(bot, top);
        return this;
    }

    public ProcessedImage<TColor, TDepth> GetComponents() {
        this.labelsCount = CvInvoke.ConnectedComponentsWithStats(
            this.currentImage,       // IInputArray image,
    ...
    public ProcessedImage<TColor, TDepth> FileterComponentsBySize(int minArea, int maxArea) {
        var iStats = this.componentsStats.ToImage<Gray, Int32>();
        var iLabels = this.componentsLabels.ToImage<Gray, Int32>();
        for (int row = 0; row < this.currentImage.Rows; row++) {
            for (int col = 0; col < this.currentImage.Cols; col++) {
                Int32 componentIdx = iLabels.Data[row, col, 0];
                if (componentIdx == 0) continue;
                Int32 componentArea = iStats.Data[componentIdx, 4, 0];
                ...
                if(minArea < componentArea && componentArea < maxArea) {
                    // white
                    this.currentImage.Data[row, col, 0] = 255;
                } else {
                    // black
                    this.currentImage.Data[row, col, 0] = 0;
                }
            }
        }
        return this;
    ...
    public ProcessedImage<TColor, TDepth> Morphology(MorphOp operation, ElementShape shape, Size kernelSize) {
        var kernel = CvInvoke.GetStructuringElement(shape, kernelSize, new Point(-1, -1));
        this.currentImage = this.currentImage.MorphologyEx(operation, kernel, new Point(-1, -1), 1, BorderType.Default, default);
        return this;
    }
}
